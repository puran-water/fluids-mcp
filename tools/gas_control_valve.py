...
from utils.constants import (
    INCH_to_M, PSI_to_PA, DEG_C_to_K
)
...
def calculate_gas_control_valve(
    ...
    try:
        ...
        if any(x is None for x in [local_gas_mw, local_gas_gamma, local_gas_z_factor, local_gas_viscosity]):
            # Attempt fluid-name-based lookup via centralized resolver (CoolProp → fluidprop → thermo)
            if fluid_name is not None:
                ...
                try:
                    ...
                    if gas_props is not None:
                        ...
                        gas_prop_source["source_info"] = "Lookup Failed"
                        prop_resolved = False
                except Exception as prop_lookup_e:
                    error_log.append(f"Warning: Property lookup failed for '{fluid_name}': {prop_lookup_e}.")
                    gas_prop_source["source_info"] = "Lookup Failed"
                    prop_resolved = False
            ...
            # If still missing viscosity, apply a default — gated on allow_property_defaults
            if local_gas_viscosity is None:
                if allow_property_defaults:
                    local_gas_viscosity = 1.8e-5
                    gas_prop_source["viscosity"] = "Default (Air)"

            # Apply MW, gamma, Z defaults if needed — gated on allow_property_defaults
            missing_props = []
